...
def analyze_waste(before_img, after_img, hunger_before, fullness_after, taste_enjoyment, reason_leftover):
    ...
    try:
        ...
        <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 16px; margin-bottom: 24px;">
            <div style="background: linear-gradient(135deg, #6d28d9, #8b5cf6); color: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 14px rgba(109, 40, 217, 0.15); text-align: center; border: 1px solid rgba(255,255,255,0.1);">
            ...
            <div style="background: linear-gradient(135deg, #b83280, #d53f8c); color: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 14px rgba(213, 63, 140, 0.15); text-align: center; border: 1px solid rgba(255,255,255,0.1);">
            ...
            <div style="background: linear-gradient(135deg, #4c1d95, #6d28d9); color: white; padding: 20px; border-radius: 12px; box-shadow: 0 4px 14px rgba(109, 40, 217, 0.15); text-align: center; border: 1px solid rgba(255,255,255,0.1);">
        ...
        for name in details.keys():
            ...
            recs_html += f"""
            <div style="margin-bottom: 12px; font-size: 13px; line-height: 1.5; padding-bottom: 8px; border-bottom: 1px solid var(--block-border-color, #edf2f7);">
                <strong style="text-transform: uppercase; color: #7c3aed;">• {name}:</strong> {rec_text}
            </div>
            """
        
        recommendations_section = f"""
        <div style="background: var(--input-background-fill, #faf5ff); border-left: 4px solid #7c3aed; padding: 22px; border-radius: 12px; margin-bottom: 24px; box-shadow: 0 4px 6px rgba(0,0,0,0.01); border-top: 1px solid var(--block-border-color, #e2e8f0); border-right: 1px solid var(--block-border-color, #e2e8f0); border-bottom: 1px solid var(--block-border-color, #e2e8f0);">
            <h4 style="margin-top: 0; color: #6d28d9; font-size: 16px; font-weight: 800; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;">💡 AI Serving Size Recommendations for Next Time</h4>
            {recs_html}
        </div>
        """
        ...
        for name, data in details.items():
            if data["leftover_percentage"] >= 5.0:
                ...
                strats_html += f"""
                <div style="margin-bottom: 14px; font-size: 13px; line-height: 1.5; padding-bottom: 10px; border-bottom: 1px dashed var(--block-border-color, #e2e8f0);">
                    <div style="font-weight: 800; text-transform: uppercase; color: #b83280; margin-bottom: 4px;">• {name}</div>
                    ...
                    <div style="color: #6b7280; font-style: italic; margin-top: 4px;">💡 Context Tip: {strat['contextual_tip']}</div>
                </div>
                """
        
        if strats_html:
            strategies_section = f"""
            <div style="background: var(--input-background-fill, #fdf4ff); border-left: 4px solid #d53f8c; padding: 22px; border-radius: 12px; margin-bottom: 24px; box-shadow: 0 4px 6px rgba(0,0,0,0.01); border-top: 1px solid var(--block-border-color, #e2e8f0); border-right: 1px solid var(--block-border-color, #e2e8f0); border-bottom: 1px solid var(--block-border-color, #e2e8f0);">
                <h4 style="margin-top: 0; color: #b83280; font-size: 16px; font-weight: 800; margin-bottom: 16px; display: flex; align-items: center; gap: 8px;">👨‍🍳 Storage & Leftover Repurposing Strategies</h4>
                {strats_html}
            </div>
            """
        else:
        ...
        macro_html = f"""
        <div style="background: var(--block-background-fill, #ffffff); padding: 22px; border-radius: 12px; border: 1px solid var(--block-border-color, #e2e8f0); margin-bottom: 24px; box-shadow: 0 4px 6px rgba(0,0,0,0.01);">
            <h4 style="margin-top: 0; font-size: 16px; font-weight: 800; margin-bottom: 18px; border-bottom: 1px solid var(--block-border-color, #edf2f7); padding-bottom: 10px; display: flex; align-items: center; gap: 8px;">📊 Macronutrients Lost Breakdown</h4>
            <div style="margin-bottom: 16px;">
                <div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 6px;">
                    <span style="font-weight: 700; color: #7c3aed;">💪 Protein Wasted</span>
                    <span style="font-weight: 800;">{prot:.1f} g</span>
                </div>
                <div style="background: #edf2f7; height: 10px; border-radius: 5px; overflow: hidden;">
                    <div style="background: #7c3aed; width: {p_pct}%; height: 100%; border-radius: 5px;"></div>
                </div>
            </div>
            <div style="margin-bottom: 16px;">
                <div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 6px;">
                    <span style="font-weight: 700; color: #d53f8c;">🍞 Carbohydrates Wasted</span>
                    <span style="font-weight: 800;">{carb:.1f} g</span>
                </div>
                <div style="background: #edf2f7; height: 10px; border-radius: 5px; overflow: hidden;">
                    <div style="background: #d53f8c; width: {c_pct}%; height: 100%; border-radius: 5px;"></div>
                </div>
            </div>
            <div>
                <div style="display: flex; justify-content: space-between; font-size: 13px; margin-bottom: 6px;">
                    <span style="font-weight: 700; color: #c084fc;">🥑 Fats Wasted</span>
                    <span style="font-weight: 800;">{fat:.1f} g</span>
                </div>
                <div style="background: #edf2f7; height: 10px; border-radius: 5px; overflow: hidden;">
                    <div style="background: #c084fc; width: {f_pct}%; height: 100%; border-radius: 5px;"></div>
                </div>
            </div>
        </div>
        """
        ...
        for name, data in details.items():
            ...
            rows_html += f"""
            <tr style="border-bottom: 1px solid var(--block-border-color, #edf2f7);">
                <td style="padding: 12px 8px; font-weight: 700; text-transform: capitalize;">{name}</td>
                ...
                <td style="padding: 12px 8px; font-weight: 700;">{data['wasted_weight_grams']:.1f}g <span style="font-size: 11px; color: #6b7280; font-weight: normal;">(of {data['baseline_weight_g']:.0f}g)</span></td>
                <td style="padding: 12px 8px; font-weight: 700; color: #d53f8c;">{data['calories_wasted']:.1f} kcal</td>
                <td style="padding: 12px 8px; font-weight: 700; color: #7c3aed;">{data['co2_wasted_kg']:.3f} kg</td>
            </tr>
            """
            
        table_html = f"""
        <div style="overflow-x: auto; background: var(--block-background-fill, #ffffff); border-radius: 12px; border: 1px solid var(--block-border-color, #e2e8f0); padding: 22px; box-shadow: 0 4px 6px rgba(0,0,0,0.01);">
            <h4 style="margin-top: 0; font-size: 16px; font-weight: 800; margin-bottom: 18px; border-bottom: 1px solid var(--block-border-color, #edf2f7); padding-bottom: 10px; display: flex; align-items: center; gap: 8px;">🍽️ Specific Ingredient Breakdown</h4>
            <table style="width: 100%; border-collapse: collapse; text-align: left; font-size: 13px;">
                <thead>
                    <tr style="border-bottom: 2px solid var(--block-border-color, #edf2f7); color: #6b7280; font-weight: 700; text-transform: uppercase; font-size: 11px; letter-spacing: 0.5px;">
