    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            with gr.Accordion("📋 Portion Learning Survey", open=True):
                hunger_before = gr.Slider(label="How hungry were you before eating? (1-10)", minimum=1, maximum=10, step=1, value=5)
                fullness_after = gr.Slider(label="How full were you after eating? (1-10)", minimum=1, maximum=10, step=1, value=8)
                taste_enjoyment = gr.Slider(label="How much did you like the meal? (1-10)", minimum=1, maximum=10, step=1, value=7)
                reason_leftover = gr.Dropdown(
                    label="Why did you leave food?",
                    choices=["None", "Too full", "Didn't taste good", "Portion too big", "Too much carb", "Too much vegetable", "Texture issue", "No time"],
                    value="None"
                )
                
            run_btn = gr.Button("Analyze Meal Waste", elem_classes="gradio-button")
...
if __name__ == "__main__":
    # Launch with custom CSS (compatible with Gradio 6.x and fallback for older versions)
    try:
        demo.launch(share=False, css=css)
    except TypeError:
        # Fallback if launch doesn't support css parameter in very old versions (though gr.Blocks support it)
        demo.css = css
